...
class Client:
    ...
    def execute_fop(self,
        ...
        persistent_fop = qiniu.PersistentFop(auth=self.auth, bucket=bucket, pipeline=pipeline, notify_url=notify_url)
        result, info = persistent_fop.execute(key=key, fops=fops, persistent_type=persistent_type,
